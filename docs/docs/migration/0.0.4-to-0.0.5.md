...
description: Migration guide from Easygram 0.0.4 to 0.0.5 — easygram property namespace, update transport config, new messaging module properties, and AnswerCallbackQuery reply API addition.
keywords: [easygram migration 0.0.5, easygram 0.0.5 upgrade, telegram bot framework breaking changes, easygram property rename, AnswerCallbackQuery easygram]
---
...
| `telegram.bot.kafka-consumer.*` | `easygram.messaging.kafka.*` + `easygram.update.transport: KAFKA_CONSUMER` |
| `telegram.bot.rabbit-consumer.*` | `easygram.messaging.rabbit.*` + `easygram.update.transport: RABBIT_CONSUMER` |
