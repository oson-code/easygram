...
- **Immutability** — reply types (`PlainReply`, `LocalizedReply`, etc.) are immutable; wither methods return new instances
- **Builder pattern** — public API types expose a static inner `Builder` class with a `builder()` factory method and `build()` terminal method
- **Template tokens** — use `#{0}`, `#{1}`, ... (0-based) for positional substitution in `PlainTextTemplate` and `LocalizedTemplate`
- **Javadoc** — all public types and methods must have Javadoc; include `@since` tag with the version it was introduced
...
3. Include:
   ...
   - Minimal reproducer (code snippet or sample project)
...
PRs are reviewed by the maintainers. Please be patient and responsive to feedback.
...
By contributing to Easygram you agree that your contributions will be licensed under the [MIT License](LICENSE).

Copyright (c) 2026 OSONCODE
