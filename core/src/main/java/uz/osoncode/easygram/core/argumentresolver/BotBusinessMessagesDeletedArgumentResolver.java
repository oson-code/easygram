...
/**
 * Argument resolver that injects the {@link BusinessMessagesDeleted} from the current update.
 *
 * @author Islom Mirsaburov
 * @since 0.0.1
 */
public class BotBusinessMessagesDeletedArgumentResolver implements BotArgumentResolver {
    ...
    public Object resolveArgument(Parameter parameter, BotRequest botRequest, BotResponse botResponse) {
        return Optional.ofNullable(botRequest.getUpdate())
                .map(Update::getDeletedBusinessMessages)
                .orElse(null);
    }
}
