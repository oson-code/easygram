...
/**
 * Default {@link BotReplyAction} that edits the originating message via
 * {@code editMessageText} / {@code editMessageReplyMarkup}.
 *
 * <p>Fires when the reply has {@code editMessage = true}, the update originates from a
 * callback query, and the reply is not a callback-alert.</p>
 *
 * @author Islom Mirsaburov
 * @since 0.0.6
 */
public class EditMessageReplyAction implements BotReplyAction {

    private final Optional<BotMarkupRegistry> markupRegistry;

    /**
     * Creates a new {@code EditMessageReplyAction}.
     *
     ...
     */
    public EditMessageReplyAction(Optional<BotMarkupRegistry> markupRegistry) {
    ...
    public boolean supports(ReplyOptions options, BotRequest request) {
        return !options.callbackAlert()
                && options.editMessage()
                && request.getUpdate().hasCallbackQuery();
    }
