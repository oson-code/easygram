...
/**
 * Integration tests for {@link EchoBotController} in the rabbit-consumer-bot sample.
 * Handler routing is tested using CoreAutoConfiguration only — no RabbitMQ broker needed.
 */
class EchoBotControllerTest {
    ...
    @Test
    void startCommand_returnsGreetingWithRabbitLabel() throws Exception {
        BotResponse response = dispatch("/start");
        assertThat(firstText(response)).contains("Hello", "Diana").contains("RabbitMQ");
    }

    @Test
    void helpCommand_returnsHelpText() throws Exception {
        BotResponse response = dispatch("/help");
        assertThat(firstText(response)).contains("/start").contains("RabbitMQ");
    }

    @Test
    void textMessage_echoesWithRabbitPrefix() throws Exception {
        BotResponse response = dispatch("bunny");
        assertThat(firstText(response)).contains("[rabbit]").contains("Diana").contains("bunny");
    }

    @Test
    void unknownUpdate_defaultHandlerFiresWithChatId() throws Exception {
        BotResponse response = dispatchUnknown(9L);
        SendMessage msg = (SendMessage) response.getBotApiMethods().iterator().next();
        assertThat(msg.getChatId()).isEqualTo("9");
    }
    ...
    private BotResponse dispatch(String text) throws Exception {
        ...
        runner.run(context -> {
            ...
            Update update = buildMessageUpdate(text, 9L, "Diana");
