...
/**
 * Integration tests for {@link EchoBotController} in the linkedlist-bot sample.
 * Tests verify handler routing independently of the in-memory queue transport.
 */
class EchoBotControllerTest {
    ...
    @Test
    void startCommand_returnsGreetingWithLinkedListLabel() throws Exception {
        BotResponse response = dispatch("/start");
        assertThat(firstText(response)).contains("Hello", "Grace").contains("LinkedList");
    }

    @Test
    void helpCommand_returnsHelpText() throws Exception {
        BotResponse response = dispatch("/help");
        assertThat(firstText(response)).contains("/start").contains("LinkedList");
    }

    @Test
    void textMessage_echoesWithLinkedListPrefix() throws Exception {
        BotResponse response = dispatch("via queue");
        assertThat(firstText(response)).contains("[linkedlist]").contains("Grace").contains("via queue");
    }

    @Test
    void unknownUpdate_defaultHandlerFiresWithChatId() throws Exception {
        BotResponse response = dispatchUnknown(20L);
        SendMessage msg = (SendMessage) response.getBotApiMethods().iterator().next();
        assertThat(msg.getChatId()).isEqualTo("20");
    }
    ...
    private BotResponse dispatch(String text) throws Exception {
        ...
        runner.run(context -> {
            ...
            Update update = buildMessageUpdate(text, 20L, "Grace");
