...
/**
 * Integration tests for {@link EchoBotController} in the kafka-consumer-bot sample.
 * Handler routing is tested using CoreAutoConfiguration only — no Kafka broker needed.
 */
class EchoBotControllerTest {
    ...
    @Test
    void startCommand_returnsGreetingWithKafkaLabel() throws Exception {
        BotResponse response = dispatch("/start");
        assertThat(firstText(response)).contains("Hello", "Carl").contains("Kafka");
    }

    @Test
    void helpCommand_returnsHelpText() throws Exception {
        BotResponse response = dispatch("/help");
        assertThat(firstText(response)).contains("/start").containsIgnoringCase("kafka");
    }

    @Test
    void textMessage_echoesWithKafkaPrefix() throws Exception {
        BotResponse response = dispatch("test message");
        assertThat(firstText(response)).contains("[kafka]").contains("Carl").contains("test message");
    }

    @Test
    void unknownUpdate_defaultHandlerFiresWithChatId() throws Exception {
        BotResponse response = dispatchUnknown(5L);
        SendMessage msg = (SendMessage) response.getBotApiMethods().iterator().next();
        assertThat(msg.getChatId()).isEqualTo("5");
    }
    ...
    private BotResponse dispatch(String text) throws Exception {
        ...
        runner.run(context -> {
            ...
            Update update = buildMessageUpdate(text, 5L, "Carl");
