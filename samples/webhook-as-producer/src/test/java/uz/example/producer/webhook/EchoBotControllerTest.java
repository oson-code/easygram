...
/**
 * Integration tests for {@link EchoBotController} in the webhook-as-producer sample.
...
class EchoBotControllerTest {
    ...
    void startCommand_returnsGreetingWithBrokerInfo() throws Exception {
        ...
        assertThat(firstText(response)).contains("Hello", "Frank").contains("broker");
    }

    @Test
    void textMessage_echoesForwardedText() throws Exception {
        BotResponse response = dispatch("route me");
        assertThat(firstText(response)).contains("Forwarded to broker").contains("route me");
    }

    @Test
    void unknownUpdate_defaultHandlerFiresWithChatId() throws Exception {
        BotResponse response = dispatchUnknown(13L);
        SendMessage msg = (SendMessage) response.getBotApiMethods().iterator().next();
        assertThat(msg.getChatId()).isEqualTo("13");
    }
    ...
    private BotResponse dispatch(String text) throws Exception {
        ...
        runner.run(context -> {
            ...
            Update update = buildMessageUpdate(text, 13L, "Frank");
